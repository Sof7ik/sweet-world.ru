...
require_once "./src/assets/php/mainpage_catalog.php";
?>
...
<section class="page-section mainpage-catalog">
	<div class="container-1440">
		...
		<div class="mainpage-catalog__items-wrapper">
			<?
//			echo "<pre>";
//				print_r($arResult["MAINPAGE_PRODUCTS"]);
//			echo "</pre>";

			foreach ($arResult["MAINPAGE_PRODUCTS"] as $product){
				$image = $product["preview_picture"] != "" ? $product["preview_picture"] : $product["detail_picture"];

//				$image = substr($image, 1, strlen($image) - 1);
//				echo $image;
				?>
				<div class="mainpage-catalog__item">
					<div class="mainpage-catalog__item-avatar" style="background-image: url(<?=$image?>);">
						<a href="/katalog/prduct?ID=<?=$product["id"]?>"
						   class="mainpage-catalog__item-detail-link">Подробнее</a>
					...
					<div class="mainpage-catalog__item-text">
						<p class="mainpage-catalog__item-price"><?=$product["price"]?> &#8381;</p>
						<p class="mainpage-catalog__item-name"><?=$product["name"]?></p>
					</div>
				</div>
			<? } ?>
